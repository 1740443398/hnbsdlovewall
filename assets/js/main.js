(function () {
        ...
        ? '<div class="post-card-author">' +
          (post.author.avatar
            ? '<img src="' +
              ...
              '" class="author-avatar" alt="头像" onerror="this.style.display=\'none\'">'
            : '') +
  ...
  function initTimeGreeting() {
    ...
    if (headerActions) {
      headerActions.insertBefore(greetingEl, headerActions.firstChild);
    }
  }
  ...
  const NotificationSystem = {
    ...
    createPanel() {
      ...
      this.panel.innerHTML =
        '<div class="notification-panel-header"><span>消息通知</span><button class="mark-all-read">全部已读</button></div>' +
        ...
        '<p>暂无通知</p></div></div>';
      this.bell.appendChild(this.panel);
    },
    ...
    bindEvents() {
      ...
      document.addEventListener('click', (e) => {
        if (this.panel.classList.contains('show') && !this.bell.contains(e.target)) {
        ...
        mentionPanel.innerHTML = filtered
          .map((u) => {
            return (
              ...
              '" alt="" onerror="this.src=\'/assets/images/default-avatar.svg\'">' +
              '<span>' +
              escapeHtml(u.nickname || '') +
              '</span>' +
              '</div>'
  ...
  function initFabButton() {
    ...
    if (fab) {
      fab.addEventListener('click', function() {
        window.location.href = SITE_URL + '/pages/post.php';
      });
    }
  }
